...
#[path = "codex_state.rs"]
mod codex_state;
const CODEX_PROMPT_SETTLE_DELAY: Duration = Duration::from_secs(2);
...
impl TerminalState {
    ...
    pub fn next_managed_agent_deadline(&self) -> Option<Instant> {
        let managed = self.managed_agent?;
        let prompt_deadline = (managed.kind == Agent::Codex
            && self.codex_prompt_ready
            && self.state != AgentState::Blocked
            && self.effective_known_agent() == Some(Agent::Codex))
        .then(|| {
            self.codex_prompt_started_at
                .and_then(|at| at.checked_add(CODEX_PROMPT_SETTLE_DELAY))
        })
        .flatten();
        match managed.phase {
            ManagedAgentPhase::Blocked => prompt_deadline,
    ...
    pub fn reconcile_managed_agent_at(&mut self, now: Instant, process_exited: bool) -> bool {
        ...
        if managed.phase == ManagedAgentPhase::Blocked {
            if known_agent == Some(managed.kind)
                && self.managed_agent_prompt_ready(managed.kind, now)
            {
                self.managed_agent = Some(ManagedAgent {
                    kind: managed.kind,
